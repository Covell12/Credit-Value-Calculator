...
if uploaded_file is not None:
    try:
        ...
        if 'risk_score' not in dataframe.columns:
            ...
            if apply_changes:
                # Define the function to calculate the RTA based on risk_score and user-defined ranges
                def calculate_rta(risk_score):
                    if risk_score <= range1:
                        ...
                        return 5  # Or any other value if the risk_score exceeds the last range

                # Apply the function to create the 'range' column based on 'risk_score'
                dataframe['range'] = dataframe['risk_score'].apply(calculate_rta)

                # Define the percentage increase based on the range
                def get_percent_increase(r):
                    if r == 1:
                        return percent1
                    elif r == 2:
                        return percent2
                ...
                dataframe['percent_increase'] = dataframe['range'].apply(get_percent_increase)
